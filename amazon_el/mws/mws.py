...
class MWS(object):
    ...
    def make_request(self, extra_data, method="GET", **kwargs):
        ...
        try:
            ...
            try:
                if 'root_key' in kwargs.keys():
                    root_key = kwargs.get('root_key')
                else:
                    root_key = extra_data.get("Action") + "Result"
                parsed_response = DictWrapper(data, root_key)
            except XMLError:
...
class Feeds(MWS):
    ...
    def get_feed_submission_result(self, feedid):
        ...
        return self.make_request(data, root_key='Message')
