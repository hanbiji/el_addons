...
class PriceRule(models.Model):
    ...
    def _get_name(self):
        for rule in self:
            ...
            name = '%s First weight: %.2fKG Added weight: %.2fKG Discount: %.2f Length: %s %.2fCM Perimeter: %s %.2fCM Surcharge: %.2f VMN: %.2f' % (name, rule.first_weight, rule.added_weight, rule.discount, rule.length_operator, rule.max_length, rule.perimeter_operator, rule.max_perimeter, rule.surcharge, rule.volumetric_weight_numerator)
            rule.name = name
    ...
    length_operator = fields.Selection([('==', '='), ('<=', '<='), ('<', '<'), ('>=', '>='), ('>', '>')], 'Length Operator', default="<=")
    max_length = fields.Float('Max Length')
    perimeter_operator = fields.Selection([('==', '='), ('<=', '<='), ('<', '<'), ('>=', '>='), ('>', '>')], 'Perimeter Operator', default="<=")
    max_perimeter = fields.Float('Max Perimeter')
    surcharge = fields.Float('Fule Surcharge')
    #check_vw = fields.Boolean('Check volumetric weight')
    volumetric_weight_numerator = fields.Integer('Volumetric weight numerator')
